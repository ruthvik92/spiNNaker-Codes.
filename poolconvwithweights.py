...
window-1
    ...
    :weight: one of the objects from the list of objects
    '''
    ...
    def PoolConv(self):
        ...
        for p in range(0,self.window-1):   
            ...
            #q = self.weight.wam[p]
            s = self.delay
            self.conn_list.append((w,v,self.weight.wam[p],s))


        for c in range(0,(self.window-1)*self.window):    
            x = self.conn_list[c][0] + self.mapp  
            y = self.conn_list[c][1]
            #q = self.weight.wam[p+1+c]
            s = self.delay
            self.conn_list.append((x,y,self.weight.wam[p+1+c],s))
        ...
        for i in range(0,extra*extra-1):     
            if(i in skip):                 
                ...
                for k in range(0+(self.window**2)*i,(self.window**2)+(self.window**2)*i):   
                    ...
                    #q = self.weight.wam[k-i*(window**2)]
                    s = self.delay
                    new_item= (n,m,self.weight.wam[k-i*(window**2)],s)
                ...
                for j in range(0+(self.window**2)*i,(self.window**2)+(self.window**2)*i):
                    ...
                    #q = self.weight.wam[j-i*(window**2)]
                    s = self.delay
                    list1 = (a,b,self.weight.wam[j-i*(window**2)],s)
