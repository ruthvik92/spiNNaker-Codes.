...
(8, 2, 1, 1), (9, 2, 1, 1), (12, 2, 1, 1), (13, 2, 1, 1), (10, 3, 1, 1), (11, 3, 1, 1), (14, 3, 1, 1), (15, 3, 1, 1)]'''
    ...
    stride = int(raw_input('Enter the stride value:'))
    extra = int(math.floor(mapp/window))
    print extra
    ...
    skip = []
    for p in range(0,(mapp)/window + extra):
        skip.append((2*(p+1)*(mapp)/window)-1)    ##these numbers say when to skip to the next row. Here the count starts from 
    print skip
                                                         ##0. map is 5x5 and window is 2x2.
    for i in range(0,((mapp+stride)/window)*((mapp+stride)/window)-1):    ####this gives number of full squares in 5x5 map.    
        if(i in skip):                  ### if we reach the count we should skip to next row.
            ...
            for k in range(0+(window**2)*i,(window**2)+(window**2)*i):   ### since our map is 2x2 there are 4 elements in the map henceforth 4.
                #print conn_list
                n = conn_list[k][0] + window*(mapp-skip[0]) -stride*mapp   ####this gives the skip 2 rows when going down. 2 rows so 2*4(added to the 5th item of previous row)  
            ...
            for j in range(0+(window**2)*i,(window**2)+(window**2)*i):
                ...
                #print i
                #print conn_list
                a = conn_list[j][0] +window-stride     ###this gives skip to next non overlapping area in the same row. map is 2x2.
                b = conn_list[j][1] +1
                q = weight
                s = delay
                list1 = (a,b,q,s)
