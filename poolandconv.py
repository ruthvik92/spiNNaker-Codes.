...
(8, 2, 1, 1), (9, 2, 1, 1), (12, 2, 1, 1), (13, 2, 1, 1), (10, 3, 1, 1), (11, 3, 1, 1), (14, 3, 1, 1), (15, 3, 1, 1)]'''
    ...
    stride = int(raw_input('Enter the overlap value:'))
    extra = int(math.floor((mapp-window)/(window-stride)+1))
    print extra
    ...
    for p in range(0,extra*extra):
        skip.append((p+1)*extra-1)    ##these numbers say when to skip to the next row. Here the count starts from 
    print skip
                                                         ##0. map is 5x5 and window is 2x2.
    for i in range(0,extra*extra-1):    ####this gives number of full squares in 5x5 map.    
        if(i in skip):                  ### if we reach the count we should skip to next row.
            ...
            for k in range(0+(window**2)*i,(window**2)+(window**2)*i):   ### since our map is 2x2 there are 4 elements in the map henceforth 4.
                ...
                print i
                print conn_list[k][0]
                z = conn_list[(window**2)*skip[0]][0]
                n = (conn_list[k][0] + mapp*(window-stride)) -z   ####this gives the skip 2 rows when going down. 2 rows so 2*4(added to the 5th item of previous row)  
